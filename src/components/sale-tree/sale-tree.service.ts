...
export class SaleTreeService {
  ...
  selectedNode: SaleTreeNode;
  targetNode: SaleTreeNode;
  rootNodes: SaleTreeNode[] = [];
  selectedNodeList: SaleTreeNode[] = [];
  expandedNodeList: SaleTreeNode[] = [];
  checkedNodeList: SaleTreeNode[] = [];
  halfCheckedNodeList: SaleTreeNode[] = [];
  matchedNodeList: SaleTreeNode[] = [];

  /**
   * reset tree nodes will clear default node list
   */
  initTree(nzNodes: SaleTreeNode[]): void {
  ...
  getSelectedNode(): SaleTreeNode | null {
  ...
  isArrayOfNzTreeNode(value: any[]): boolean {
    return value.every(item => item instanceof SaleTreeNode);
  }

  /**
   * reset selectedNodeList
   */
  calcSelectedKeys(selectedKeys: string[], nzNodes: SaleTreeNode[], isMulti: boolean = false): void {
    this.selectedNodeList = [];
    const calc = (nodes: SaleTreeNode[]) => {
      nodes.forEach(node => {
   ...
   */
  calcExpandedKeys(expandedKeys: string[], nzNodes: SaleTreeNode[]): void {
    this.expandedNodeList = [];
    const calc = (nodes: SaleTreeNode[]) => {
      nodes.forEach(node => {
   ...
   */
  calcCheckedKeys(checkedKeys: string[], nzNodes: SaleTreeNode[], isCheckStrictly: boolean = false): void {
    this.checkedNodeList = [];
    this.halfCheckedNodeList = [];
    const calc = (nodes: SaleTreeNode[]) => {
      nodes.forEach(node => {
   ...
   * set drag node
   */
  setSelectedNode(node?: SaleTreeNode): void {
   ...
   */
  setNodeActive(node: SaleTreeNode, isMultiple: boolean = false): void {
   ...
   */
  setSelectedNodeList(node: SaleTreeNode, isMultiple: boolean = false): void {
   ...
   * merge checked nodes
   */
  setHalfCheckedNodeList(node: SaleTreeNode): void {
  ...
  setCheckedNodeList(node: SaleTreeNode): void {
   ...
   */
  conductCheck(type: string = 'check'): SaleTreeNode[] {
    const checkedNodeList = [];
    const loop = (node: SaleTreeNode) => {
      switch (type) {
        case 'check':
          if (node.isChecked) {
   ...
   * set expanded nodes
   */
  setExpandedNodeList(node: SaleTreeNode): void {
    if (node.isLeaf) {
      return;
    }
  ...
  conduct(node: SaleTreeNode): void {
   ...
   */
  conductUp(node: SaleTreeNode): void {
   ...
   */
  conductDown(node: SaleTreeNode, value: boolean): void {
  ...
  searchExpand(value: string): void {
    ...
    this.expandedNodeList = [];
    const expandParent = (p: SaleTreeNode) => {
      // expand parent node
      if (p.getParentNode()) {
    ...
    const searchChild = (n: SaleTreeNode) => {
   ...
   * drag event
   */
  refreshDragNode(node: SaleTreeNode): void {
    if (node.getChildren().length === 0) {
      // until root
      this.conductUp(node);
    } else {
  ...
  // reset node level
  resetNodeLevel(node: SaleTreeNode): void {
   ...
   */
  dropAndApply(targetNode: SaleTreeNode, dragPos: number = -1): void {
   ...
   * dragNode
   */
  formatEvent(eventName: string, node: SaleTreeNode, event: MouseEvent | DragEvent): SaleFormatEmitEvent {
    const emitStructure = {
      'eventName': eventName,
      'node'     : node,
      'event'    : event
